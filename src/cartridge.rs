...
impl Rom {
    pub fn new(raw: &Vec<u8>) -> Result<Rom, String> {
        ...
        println!("{} {} {} {}", raw[prg_rom_start], raw[prg_rom_start+1], raw[prg_rom_start+2], raw[prg_rom_start+3]);
        Ok(Rom {
 ...
 pub mod test {
    ...
    pub fn test_rom(program: Vec<u8>) -> Rom {
        let mut rom: Vec<u8> = vec![1; 2 * PRG_ROM_PAGE_SIZE];
        rom[0] = 0xa9;
        rom[1] = 0x05;

        let blah: TestRom = TestRom {
            header: vec![
        ...
        let test_rom = create_rom(blah);

        println!("Number Of Cars : {}", program.len());
